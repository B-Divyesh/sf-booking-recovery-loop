...
export interface Practice { id: string; name: string; publicSlug: string; timezone: string; serviceName: string; durationMinutes: number; depositCents: number; currency: string; checkoutProvider: string; deliveryConnected: boolean; attempts: PracticeAttempt[]; }
export interface PublicPractice { name: string; publicSlug: string; timezone: string; serviceName: string; durationMinutes: number; depositCents: number; currency: string; checkoutProvider: string; consentWording: string; }
...
export async function createBookingAttempt(slug: string, payload: Record<string, unknown>): Promise<{ attemptId: string; checkoutUrl: string; checkoutIntentId: string; status: string }> {
...
export async function completeBookingPayment(attemptId: string, license: string): Promise<{ recorded: boolean; duplicate: boolean }> {
  return responseJson(await fetch(`/api/v1/public/attempts/${encodeURIComponent(attemptId)}/payments/complete`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ license }) }));
}
